...
#define ELF_ENTRY_OFFSET 24 /* elfclass32 and elfclass64 ehdr have
			     * interchangable offset up until the e_entry member
			     */
/*
...
#define ELF_PT_LOAD_MISC_F (1 << 2)
	...
	/*
	 * Userland exec specific stuff, including lists.
	 */
	uint64_t ldso_entry;
	uint64_t ldso_base;
	struct {
		SLIST_HEAD(elf_mapping_list, elf_mapping_node) mappings;
		SLIST_HEAD(elf_ldso_mapping_list, elf_mapping_node) ldso_mappings;
	} slist;
...
bool elf_map_binary(elfobj_t *, elf_error_t *);
...
/*
 * userland exec, userland debugging, etc.
 */
bool elf_ul_exec(struct elfobj *, int, char **, char **, elf_error_t *);
#endif
